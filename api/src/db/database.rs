...
impl Database {
    ...
    pub async fn get_recent_matches(&self, limit: i64) -> Option<HashMap<u32, MatchData>> {
        let result = sqlx::query!(
            r#"
     SELECT m.match, m.server, m.start_time, m.duration, m.winner, m.team_one_score, m.team_two_score,
       m.map, m.is_tourney, m.team_one_name, m.team_two_name, m.team_one_color, m.team_two_color,
     ...
     "#,
        ...
        match result {
            ...
            Err(e) => {
                warn!("Error retrieving recent matches {e:?}");
                None
            }
        }
    }
    ...
    pub async fn get_uuid_from_username(&self, username: &str) -> Option<Uuid> {
        let lower_username = username.to_lowercase();
        let result = sqlx::query!(
            r#"SELECT uuid FROM player_identities WHERE LOWER(name) = $1 LIMIT 1"#,
            lower_username
        )
        .fetch_optional(&self.connection_pool)
        .await;
        match result {
            Ok(Some(record)) => Some(Self::parse_uuid(record.uuid)),
            Ok(None) => None,
            Err(e) => {
                warn!("Error looking up username {username}: {e:?}");
                None
            }
        }
    }
    ...
    pub async fn get_player_all_match_stats(
        &self,
        uuid: Uuid,
    ) -> Option<Vec<(PlayerMatchStats, u32)>> {
        let uuid_bytes = uuid.as_bytes().to_vec();
        let uuid_hyphenated = uuid.hyphenated().to_string();
        let uuid_string_bytes = uuid_hyphenated.as_bytes();
        let result = sqlx::query!(
            r#"
            SELECT p.team, p.kills, p.deaths, p.assists, p.killstreak, p.dmg_dealt, p.dmg_taken,
                   p.pickups, p.throws, p.passes, p.catches, p.strips, p.touchdowns,
                   p.touchdown_passes, p.passing_blocks, p.receive_blocks,
                   p.defensive_interceptions, p.pass_interceptions, p.damage_carrier,
                   m.duration
            FROM player_match_data p
            JOIN match_data m ON p.match = m.match
            WHERE (p.player = $1 OR p.player = $2) AND m.start_time > 0
            "#,
            uuid_bytes.as_slice(),
            uuid_string_bytes,
        )
                    ...
                    rows.iter()
                        .map(|record| {
                            let stats = PlayerMatchStats {
                                ...
                                passing_blocks: record.passing_blocks.unwrap_or(0.0) as f32,
                                receive_blocks: record.receive_blocks.unwrap_or(0.0) as f32,
                                defensive_interceptions: record
                                    .defensive_interceptions
                                    .unwrap_or(0.0)
                                    as u32,
                                pass_interceptions: record.pass_interceptions.unwrap_or(0.0)
                                    as u32,
                                damage_carrier: record.damage_carrier.unwrap_or(0.0) as f32,
                            };
                            (stats, record.duration as u32)
                        })
                        .collect(),
                )
            }
            Err(e) => {
                warn!("Error retrieving player aggregate stats {e:?}");
                None
            }
        }
    }
